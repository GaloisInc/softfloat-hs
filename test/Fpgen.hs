...
parseSubnormal32 = do
  sign <- Parsec.choice
    [Parsec.char '+' >> return zero32, Parsec.char '-' >> return minus32]
  _              <- Parsec.string "0."
  significandRaw <- Parsec.manyTill (Parsec.digit <|> Parsec.oneOf "ABCDEF")
                                    (Parsec.string "P-126")
...
parseFloat = do
  ...
  number <-
    Parsec.many1 (Parsec.noneOf [' ']) <?> "parseFloat : cannot parse a number"
...
parseTypes = do
  ...
  -- get inputs
  inputsFloat <-
    Parsec.manyTill parseFloat (Parsec.try $ Parsec.string "->")
      <?> "cannot parse inputs"
...
checkOpArguments fp = case _operation fp of
                    -- 1 argument
  op
    | op
      `elem` [ Abs
...
executeOperation fp = do
 ...
 where
  evalResults fpgenResult softFloatResult = evalTest
    (fpgenResult == softFloatResult)
    (  "Results are not matching.\n\n"
    ++ "Fpgen:     "
    ++ show fpgenResult
    ++ "\n"
    ++ "Softfloat: "
    ++ show softFloatResult
    ++ "\n\n"
    ++ show fp
    )
