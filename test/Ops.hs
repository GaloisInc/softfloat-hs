...
instance Display AnyResult where
    display (RF16 r) = display r
    display (RF32 r) = display r
    display (RF64 r) = display r
    display (RBool r) = display r
    display (RI32 r) = display r
    display (RI64 r) = display r
...
isCompare arg =
  ("_le" `isInfixOf` arg) || ("_lt" `isInfixOf` arg) || ("_eq" `isInfixOf` arg)
...
readResult operands arg
  | isCompare arg         = RBool (Result (head $ opsBool operands) flags)
...
parseOp operands args
  | isConvert $ head args          = FConvert $ parseConvertOp operands args
...
parseConvertOp operands args
  | opType == "ui32_to_f16" = Ui32ToF16 r (ops32 operands !! 0)
...
parseOpInner ops args
  | opType == "add"          = Arith $ Add r (ops !! 0) (ops !! 1)
...
instance Display F16Result where
  display (Result r ex) = "0x" ++ binPadded ++ " " ++ display ex
    where
        binRaw = (showIntAtBase 16 intToDigit r "")
        binPadded = (replicate (4 - length binRaw) '0') ++ binRaw

instance Display F32Result where
  display (Result r ex) = "0x" ++ binPadded ++ " " ++ display ex
    where
        binRaw = (showIntAtBase 16 intToDigit r "")
        binPadded = (replicate (8 - length binRaw) '0') ++ binRaw

instance Display I32Result where
  display (Result r ex) = "0x" ++ binPadded ++ " " ++ display ex
    where
        binRaw = (showIntAtBase 16 intToDigit r "")
        binPadded = (replicate (8 - length binRaw) '0') ++ binRaw

instance Display F64Result where
  display (Result r ex) = "0x" ++ binPadded ++ " " ++ display ex
    where
        binRaw = (showIntAtBase 16 intToDigit r "")
        binPadded = (replicate (16 - length binRaw) '0') ++ binRaw

instance Display I64Result where
  display (Result r ex) = "0x" ++ binPadded ++ " " ++ display ex
    where
        binRaw = (showIntAtBase 16 intToDigit r "")
        binPadded = (replicate (16 - length binRaw) '0') ++ binRaw

instance Display BoolResult where
  display (Result r ex) = show r ++ " " ++ display ex

instance Display ExceptionFlags where
  display (ExceptionFlags x u o z i) = (displayFlag x "x") ++
                                       (displayFlag u "u") ++
                                       (displayFlag o "o") ++
                                       (displayFlag z "z") ++
                                       (displayFlag i "i")
    where
      displayFlag v s | v == True = s
                  | otherwise = ""
